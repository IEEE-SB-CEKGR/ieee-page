...
export default function MemberCard({ member }: { member: any }) {
  ...
  return (
    <motion.div
      className="group relative h-96 w-72 overflow-hidden rounded-xl shadow-2xl"
    ...
    >
      {/* Decorative elements */}
      <div className="absolute -right-20 -top-20 z-0 h-40 w-40 rounded-full bg-accent/20 opacity-0 blur-3xl transition-opacity duration-700 group-hover:opacity-100" />
      <div className="absolute -bottom-20 -left-20 z-0 h-40 w-40 rounded-full bg-cyan-500/20 opacity-0 blur-3xl transition-opacity duration-700 group-hover:opacity-100" />

      {/* Card border glow effect */}
      <motion.div
        className="absolute inset-0 z-10 overflow-hidden rounded-xl border border-white/10"
        animate={{
          boxShadow: isHovered
      ...
      {/* Role badge */}
      <motion.div
        className="absolute right-6 top-6 z-20 rounded-full bg-accent/90 px-3 py-1 backdrop-blur-sm"
        initial={{ opacity: 0, y: -20 }}
        animate={{
      ...
      >
        ...
        <motion.div>
          <motion.div
            className="mb-3 h-0.5 w-12 rounded bg-accent"
          ...
          <motion.p
            className="mb-5 font-medium text-cyan-300 opacity-90"
        ...
        >
          {member.linkedin && (
            <motion.a
              ...
              whileTap={{ scale: 0.95 }}
            >
              <div className="rounded-full bg-white/10 p-2 transition-all hover:bg-white/20">
